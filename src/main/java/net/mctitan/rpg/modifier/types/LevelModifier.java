...
import net.mctitan.rpg.modifier.templates.LevelTemplate;
import net.mctitan.rpg.util.Key;
import net.mctitan.rpg.util.Math;
import org.bukkit.persistence.PersistentDataContainer;
import org.bukkit.persistence.PersistentDataType;

public class LevelModifier extends Modifier {
    private Operator operator;
    private double value;

    public LevelModifier() {}

    public LevelModifier(int rank, LevelTemplate template, boolean base, boolean monster,
    ...
    public LevelModifier(PersistentDataContainer pdc) {
        super(pdc, new LevelTemplate(pdc.get(Key.TEMPLATE_KEY,  PersistentDataType.TAG_CONTAINER)));
    ...
    public Component text() {
        if(operator() == Operator.FLAT) {
            String valuestr = String.format("%s%.00f", operator().string(value(), base()), net.mctitan.rpg.util.Math.abs(value()));
            return Component.text(String.format("%s Level", valuestr));
        } else if(operator() == Operator.SCALER || operator() == Operator.MULTIPLIER) {
            String valuestr = String.format("%.00f", Math.abs(value()));
            return Component.text(String.format("%s%% %s Level", valuestr, operator().string(value(), base())));
        }

        return Component.text(String.format("Undefined Level %s %.03f", operator(), value()));
    }
