...
import net.mctitan.rpg.modifier.types.HealingModifier;
...
public class HealingTemplate extends ModifierTemplate {
    private Operator operator;
    private double precision;
    private double flat;
    private int steps;

    public HealingTemplate(int rank, boolean base, boolean monster, String id, Operator operator,
    ...
    public HealingTemplate(PersistentDataContainer pdc) {
        super(pdc);
    ...
    public HealingTemplate(ConfigurationSection section) {
        super(section);
    ...
    public Modifier modifier(Random random) {
        ...
        return new HealingModifier(rank(), this, base(), monster(), operator(), value);
    }
    ...
    public String string() {
        ...
        if(operator() == Operator.FLAT) {
            String valuestr = String.format("%.01f", min);
            if(steps() > 1) { valuestr = String.format("(%s - %.01f)", valuestr, max); }
            return String.format("Heals %s health", valuestr);
        }

        if(steps() > 1) {
            return String.format("Undefined Healing %s %.03f %.03f", operator(), min, max);
        } else {
            return String.format("Undefined Healing %s %.03f", operator(), min);
        }
    }
