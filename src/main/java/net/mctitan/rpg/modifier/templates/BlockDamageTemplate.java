...
public class BlockDamageTemplate extends ModifierTemplate {
    private Operator operator;
    private double precision;
    private double flat;
    private int steps;

    public BlockDamageTemplate(int rank, boolean base, boolean monster, String id, Operator operator,
    ...
    public BlockDamageTemplate(PersistentDataContainer pdc) {
        super(pdc);
    ...
    public BlockDamageTemplate(ConfigurationSection section) {
        super(section);
    ...
    public Modifier modifier(Random random) {
        ...
        return new BlockDamageModifier(rank(), this, base(), monster(), operator(), value);
    }
    ...
    public String string() {
        ...
        if(operator() == Operator.FLAT) {
            String valuestr = String.format("%s%.00f", operator().string(min, base()), min);
            if(steps() > 1) { valuestr = String.format("(%s - %s%.00f)", valuestr, operator().string(max, base()), max); }
            return String.format("%s%% Blocked Damage", valuestr);
            ...
            return String.format("%s%% %s Blocked Damage", valuestr, operator().string(min, base()));
        } else if(operator() == Operator.SET) {
            String valuestr = String.format("%s%.00f", operator().string(min, base()), min);
            if(steps() > 1) { valuestr = String.format("(%s - %s%.00f)", valuestr, operator().string(max, base()), max); }
            return String.format("Blocked Damage is %s%%", valuestr);
        }

        return String.format("Undefined Block Damage %s %.03f", operator().name(), min);
    }
