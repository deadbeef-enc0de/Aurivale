...
public class LevelTemplate extends ModifierTemplate {
    private Operator operator;
    private double precision;
    private double flat;
    private int steps;

    public LevelTemplate(int rank, boolean base, boolean monster, String id, Operator operator,
    ...
    public LevelTemplate(PersistentDataContainer pdc) {
        super(pdc);
    ...
    public LevelTemplate(ConfigurationSection section) {
        super(section);
    ...
    public Modifier modifier(Random random) {
        ...
        return new LevelModifier(rank(), this, base(), monster(), operator(), value);
    }
    ...
    public String string() {
        ...
        if(operator() == Operator.FLAT) {
            ...
            return String.format("%s Level", valuestr);
            ...
            return String.format("%s%% %s Level", valuestr, operator().string(min, base()));
        }

        if(steps() > 1) {
            return String.format("Undefined Level %s %.03f %.03f", operator(), min, max);
        } else {
            return String.format("Undefined Level %s %.03f", operator(), min);
        }
    }
